...
(function(){
	...
	BX.Tasks.Component.TaskView = function(parameters)
	{
		...
		this.statusQuery = new BX.Tasks.Util.Query({url: "/local/ajax/task_ajax.php"}); /* Edited */
	...
	BX.Tasks.Component.TaskView.prototype.onImportantButtonClick = function(node, event)
	{
		var curPriority, newPriority;
		
		if (event.target.nodeName == 'INPUT' && event.target.name == 'task-edit-priority')
		{
		    curPriority = BX.data(node, 'priority');
		    newPriority = +event.target.value;
		    
		    //Сразу ставим новую критичность.
		    //Если в запросе вернется ошибка - вернем обратно
		    // если ошибки нет, значит все ок
		    BX.data(node, 'priority', newPriority);
		    $(node).attr('data-priority', newPriority);
			...
			if(!result.isSuccess())
			{
				BX.data(node, 'priority', curPriority);
				$(node).attr('data-priority', curPriority);
			}
		    }.bind(this));
		    this.query.execute();
		}
