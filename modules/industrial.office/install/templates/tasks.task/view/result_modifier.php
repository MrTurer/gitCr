...
use Bitrix\Tasks\UI;
use Bitrix\Tasks\Util; /* Inserted */
use \Bitrix\Tasks\Util\Type;
use \Bitrix\Highloadblock\HighloadBlockTable;
use RNS\Industrial\Office\Helpers\Helper;
...
if (!CModule::IncludeModule('industrial.office')){
	Loc::getMessage("RNS_MODULE_NOT_INSTALL");
	return;
}

$intEntityID = (isset($taskData['UF_TYPE_ENTITY'])) ? $taskData['UF_TYPE_ENTITY'] : 0; /* Inserted */
...
/* $userFields = isset($arResult["AUX_DATA"]["USER_FIELDS"]) ? $arResult["AUX_DATA"]["USER_FIELDS"] : array(); */ /* Committed */

/* Begin */
if (isset($arResult["AUX_DATA"]["USER_FIELDS"])) {
	$userFields = array();
	$arResult['DATA']['TASK']['BOUND_FIELDS'] = Helper::getBoundFields($intEntityID);

	$arTmp = $arResult["AUX_DATA"]["USER_FIELDS"];
	$arResult["AUX_DATA"]["USER_FIELDS"] = array();
	
	foreach($arResult['DATA']['TASK']['BOUND_FIELDS'] as $strFieldCode => $arField) {
		if (isset($arTmp[$strFieldCode]) ) {
			$arResult["AUX_DATA"]["USER_FIELDS"][$strFieldCode] = $arTmp[$strFieldCode];
		}
	}
	unset($arTmp);
...
/* Begin */
$arResult['DATA']['TASK']['STATUSES'] = Helper::getStatuses($intEntityID, $arResult['DATA']['TASK']['UF_STATUS']);
$arResult['DATA']['TASK']['USE_TIME'] = Helper::checkUsingTime($intEntityID);

$arResult['VIEW_URL'] = UI\Task::makeActionUrl($arParams["PATH_TO_TASKS_TASK"], $arResult["DATA"]["TASK"]["ID"], 'view');
$arResult['EDIT_URL'] = UI\Task::makeActionUrl($arParams["PATH_TO_TASKS_TASK"], $arResult["DATA"]["TASK"]["ID"], 'edit');
