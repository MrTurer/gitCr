...
$APPLICATION->SetAdditionalCSS("/local/templates/bitrix24/components/bitrix/tasks.task.detail.parts/flat/sidebar/style.css");
		...
		<div class="task-info">
			<div class="task-info-panel">
				<div class="task-info-panel-important">
				    <span class="task-info-important-label"><?=Loc::getMessage('TASKS_TASK_COMPONENT_TEMPLATE_PRIORITY')?></span>
				    <span class="priority-toggler-wrapper">
					<?
					$arrPriorities = [\CTasks::PRIORITY_LOW, \CTasks::PRIORITY_AVERAGE, \CTasks::PRIORITY_HIGH];
					foreach($arrPriorities as $priority)
					{
					    $inputId = 'priority-toggler-input-'.$priority;
					?>
					
					    <input data-bx-id="task-edit-priority-cb" id="<?=$inputId?>" class="priority-toggler-input" type="radio" name="task-edit-priority" value="<?=$priority?>" <?=($taskData['PRIORITY'] == $priority ? 'checked' : '')?>>
					    <label for="<?=$inputId?>" class="priority-toggler-label" title="<?=Loc::getMessage('TASKS_PRIORITY_'.$priority)?>"></label>
					
					<?
					    }
					?>
					<span class="priority-toggler-fill"></span>
				    </span>
				    <input class="fire-toggler" data-bx-id="task-edit-priority" type="hidden" name="<?=htmlspecialcharsbx($inputPrefix)?>[PRIORITY]" value="<?=intval($taskData['PRIORITY'])?>" />
				</div>
