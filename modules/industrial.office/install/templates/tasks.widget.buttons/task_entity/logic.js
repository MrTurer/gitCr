...
(function() {
	if (typeof BX.Tasks.Component.Statuses != 'undefined')
	{
		return;
	}

	BX.Tasks.Component.Statuses = function(parameters)
	{
		this.params = parameters || {};
	};
	
	BX.Tasks.Component.Statuses.prototype.getParams = function()
	{
		return this.params;
	};
	...
	BX.Tasks.Component.TasksWidgetButtonsTask = BX.Tasks.Util.Widget.extend({
		sys: {
			code: 'task-view-b',
			codeStatusMenu: 'task-view-statuses'
		},
		options: {
		...
		methods: {
			...
			bindEvents: function()
			{
				...
				this.bindDelegateControl('open-change-statuses', 'click', this.passCtx(this.onMenuChangeStatusesOpen));
			},
			...
			onMenuChangeStatusesOpen: function(node)
			{
				if(!this.vars.buttonActionEnabled)
				{
					return;
				...
				BX.PopupMenu.show(
					this.sys.codeStatusMenu,
					node,
					this.getStatusesMenu(),
					{
						closeByEsc: true
					}
				);
			...
			getStatusesMenu: function()
			{
				var menu = [];
				
				if (typeof obStatuses != 'undefined')
				{
					var obStatusesList = obStatuses.getParams();
					
					if (this.nextStatus != '') {
						var	obNextStatuses = obStatusesList.ALL_CUSTOM_STATUSES[this.nextStatus].UF_NEXT_STATUS;
					} else {
						var	obNextStatuses = obStatusesList.CUSTOM_STATUS.UF_NEXT_STATUS;
					}
					
					var obAllowStatuses = obStatusesList.ALL_CUSTOM_STATUSES,
						obStatus = {};
					
					for (var i in obNextStatuses) {
						obStatus = obAllowStatuses[obNextStatuses[i]];
						
						if (obStatus.UF_NEXT_STATUS_BUTTON_NAME[obNextStatuses[i]] != '') {
							obStatus.NAME = obStatus.UF_NEXT_STATUS_BUTTON_NAME[obNextStatuses[i]];
						}
						
						menu.push(
							{
								code: obNextStatuses[i],
								text: obStatus.NAME,
								title: obStatus.NAME,
								className: 'menu-popup-item',
								onclick: this.passCtx(this.onStatusButtonClick),
								statusID: obStatus.ID,
							},
						);
					};
				};
			...
			onStatusButtonClick: function (menu, e, item)
			{
				var code = item.code,
				...
				if (code && statusID > 0)
				{
					...
					this.query.run('task.update', {
						...
						if(!result.isSuccess()) {
							...
							BX.PopupMenu.destroy(this.sys.codeStatusMenu);
							this.nextStatus = code;
						}
					}.bind(this));
					this.query.execute();
				}
